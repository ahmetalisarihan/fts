...
export async function PUT(
  ...
  try {
    ...
    const whereClause =
      params.slug.length === 24 && /^[0-9a-fA-F]{24}$/.test(params.slug)
    ...
    // Cache'leri akıllı şekilde temizle (update işlemi için skip)
    ...
    console.log(error);
    return NextResponse.json({
      message: "Bir düzenleme hatası oluştu. Lütfen tekrar deneyin.",
    });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    // Cache'leri temizle (delete işlemi kritik)
    const { shouldInvalidateCache } = await import('@/utils/cache-strategy');
    ...
    console.log(error);
    return NextResponse.json({
      message: "Silme isleminde bir hata oluştu. Lütfen tekrar deneyin.",
    });
  }
}
