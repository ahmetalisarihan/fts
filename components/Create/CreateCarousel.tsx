    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('/api/carousels', {
                ...
                body: JSON.stringify({ imageUrl, carouselLink }),
            });

            if (!response.ok) {
                const errorData = await response.json();
                setError(errorData.error || 'Carousel öğesi oluşturulamadı.');
            } else {
                setSuccess('Carousel öğesi başarıyla oluşturuldu!');
                setImageUrl('');
                setCarouselLink('');
            }
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className='space-y-2'>
                <Input onChange={e => setCarouselLink(e.target.value)} type="text" placeholder="Carousel Linkini Yapıştırınız" value={carouselLink} />
            ...
            <Button type='submit' variant='default' size='default' className='max-w-[250px] m-auto'>Carousel Oluştur</Button>
